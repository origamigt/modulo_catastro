...
public class ProxyServlet extends HttpServlet {
    ...
    public void init() throws ServletException {
        ...
        String doForwardIPString = this.getConfigParam(P_FORWARDEDFOR);
        if (doForwardIPString != null) {
            this.doForwardIP = Boolean.parseBoolean(doForwardIPString);
        }

        this.initTarget();// sets target*

        HttpParams hcParams = new BasicHttpParams();
        this.readConfigParam(hcParams, ClientPNames.HANDLE_REDIRECTS, Boolean.class);
     ...
     * many opportunities for customization. By default, SystemDefaultHttpClient
     * is used available, otherwise it falls back to:
     *
     * <pre>
    ...
    public void destroy() {
        ...
        if (this.proxyClient instanceof Closeable) {// AutoCloseable in Java 1.6
            try {
    ...
    protected void service(HttpServletRequest servletRequest, HttpServletResponse servletResponse)
        ...
        try {
            // Execute the request
            if (this.doLog) {
                this.log("proxy " + method + " uri: " + servletRequest.getRequestURI() + " -- "
                        + proxyRequest.getRequestLine().getUri());
            }
            proxyResponse = this.proxyClient.execute(this.getTargetHost(servletRequest), proxyRequest);

            // Process the response
            int statusCode = proxyResponse.getStatusLine().getStatusCode();

            if (this.doResponseRedirectOrNotModifiedLogic(servletRequest, servletResponse, proxyResponse, statusCode)) {
    ...
    protected String rewriteUrlFromResponse(HttpServletRequest servletRequest, String theUrl) {
        // document example paths
        final String targetUri = this.getTargetUri(servletRequest);
